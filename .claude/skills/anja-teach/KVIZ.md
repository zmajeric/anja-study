...
resource page around them. Name the set `notes-<colour or kind>-<date and time>`
(`notes-ink-2026-09-24-1430`) and give every question `notes: [<note ids>]`, which feeds the quiz
pages' "From my notes" filter.

## 2. Write the questions

Add them as a new set named with the date and time of writing, `YYYY-MM-DD-HHMM` (`2026-09-24-1430`;
no colon, as the name is also a file name and a URL value), in a new file
`assets/bank/<chapter>-<set>.js`, per [BANKS.md](./BANKS.md). A mixed request splits `n` by the
subject's question mix; image questions need images the resources contain.
Add the file to `subject.json` → `extraBanks`, and label the set in `subject.json` → `sets` as
`"Kviz DD-MM-YY HH:MM"` (`"Kviz 24-09-26 14:30"`), the format the custom quiz uses.
...
`quiz/image.html?subject=histology&chapter=ch5&set=2026-09-24-1430` (published site or local server).
